...
Landed: R7-10 STOP (catalog has no CLI producer); R7-6 Round 6 docs truth-up (ADR-006 stays Proposed); R7-7 deleted unused `settleSupervisedOutcome`; R7-9 empty graphs fail in pre-flight; R7-3 absorbing-BLOCKED pins + Round 8 `RUN_UNBLOCKED` design (no schema); R7-8 keyed `loadProjectBanditByKey` + pure preference reader.
