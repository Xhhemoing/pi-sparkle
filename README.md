...
`pi-sparkle auth login openai --key sk-...` writes `~/.pi-sparkle/runtime/auth.json` (stored credentials win over env). `PI_PROVIDER` / `PI_MODEL` / `PI_API_KEY` still work as a compatibility override for the default provider.
...
| `pnpm cli adapt eval` / `adapt rollback` | Implemented on the adaptation CLI (`pi-sparkle adapt help`). Not a live-loop mutation; eval still needs a dataset the auto-loop does not yet export |
