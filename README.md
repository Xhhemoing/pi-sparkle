...
| `pnpm cli unblock --run <runId> --reason <text>` | The only event that clears a BLOCKED run: it records the authorization and reopens the state, then `resume` executes the reopened work — unblock itself runs nothing. `--retry-node <nodeId>` re-drives one FAILED flowchart node (a stall block takes no node). Rewinding a descendant that already executed is refused unless `--discard-executed` authorizes it: a separate authorization, requiring the exact `--retry-node`, whose discarded set is computed under the run lock rather than listed, and no budget is refunded |
