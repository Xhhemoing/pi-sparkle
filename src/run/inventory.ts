...
export async function listRuns(stateRoot: string): Promise<RunInventory> {
  ...
  try {
    ...
    if (isMissingDirectory(error)) return { runs: [], errors };
    throw error;
  }
...
export async function listEpisodes(stateRoot: string): Promise<EpisodeInventory> {
  ...
  try {
    ...
    if (isMissingDirectory(error)) return { episodes: [], errors };
    throw error;
  }
  ...
  for (const child of children) {
    ...
    try {
      const latest = (await new EpisodeStore(stateRoot, episodeId).readAll()).episodes.at(-1);
