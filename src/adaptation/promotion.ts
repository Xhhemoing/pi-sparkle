...
import { assertRoutingPolicyEvalReport, isIntentId, validatePromotionReview } from "./promotion-rules.js";
...
export async function loadAdaptationRegistry(
  ...
  return ResourceRegistry.fromSnapshot(snapshot, options);
}

/**
 * Load the persisted registry, or start an empty one when no snapshot has
 * been written yet. Any other load failure still fails closed.
 */
export async function loadAdaptationRegistryOrNew(
  stateRoot: string,
  options?: RegistryOptions
): Promise<ResourceRegistry> {
  try {
    return await loadAdaptationRegistry(stateRoot, options);
  } catch (error) {
