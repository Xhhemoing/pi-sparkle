...
- [x] Apply registration slice (`sparkle_apply_candidate`) implemented 2026-09-20 on `feat/native-apply-registration` (`1707c8f`): tool consumes only a host-issued handle; trusted result reconstructed from hash-verified loop-artifact bytes; issuance via host-only `/sparkle-issue-candidate`; tool surface pinned. Focused unit 23/23, integration 15/15, serialized full suite 2816 pass / 0 fail / 18 skip; typecheck/lint/build/security/Pi probes pass. [Verification record](../docs/reports/2026-09-20-native-apply-registration.md). [ ] Pending independent review and owner authorization (G3) before merge.
