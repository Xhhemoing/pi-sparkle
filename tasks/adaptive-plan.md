...
- [ ] User turns, approved specs/plans/ADRs, and repository facts retain distinct authority/provenance.
- [ ] Prompt-like text in code, logs, quoted web content, or tool output is data and cannot become user authority.
- [ ] Extraction emits explicit objectives, deliverables, constraints, non-goals, assumptions, questions, authority grants, acceptance criteria, and source references.
- [ ] A critic receives normalized sources plus the candidate, not the extractor's defense, and reports omissions/contradictions without mutating it.
- [ ] Low-confidence or materially conflicting output routes to a user decision; it does not silently pick an interpretation.
- [ ] Inferred/latent requirements are labeled as inference and routed to user confirmation unless corroborated by an explicit source.
- [ ] Faux-provider fixtures reproduce extraction/critique protocol behavior without credentials.
...
```bash
corepack pnpm exec tsx --test test/unit/requirements/source-normalization.test.ts test/integration/m3/requirement-extraction.test.ts
corepack pnpm run typecheck
```
...
- [ ] A ranked code-map view (public interfaces and call structure) with its own token budget complements the file-level facts.
...
**Commands:** extend CLI with `episode inspect <id>`, `episode events <id> --json`, and `episode close <id>`; exact argument style must match the implemented CLI parser.

**Acceptance:**

- [ ] Closure rejects uncovered criteria, unresolved blockers, invalid evidence, or missing required checks.
- [ ] Inspection distinguishes observed, failed, and unobserved dimensions.
- [ ] The CLI never labels an agent's completion claim as verified without evidence.
- [ ] Restarting the CLI preserves episode/run linkage and terminal state.
