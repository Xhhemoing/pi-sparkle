...
| Capability | Present | Wired | Exercised | Outcome-supported | Notes |
|---|---|---|---|---|---|
| R0 / static `ModelRouter` | yes | live flowchart + `--children` assign | yes | no | Live path. `routeFlowNode` runs `analyzeTask` and persists compile-time `agentRole` (`flowchart-v4`). High-risk analysis arms `WAITING_FOR_USER`. `--assume-defaults` auto-selects default approval items and labels them `answeredBy: assume-defaults-auto` (absent field = legacy). Stage 2 explicit `--approve-high-risk` needs owner sign-off; see [strategy](reports/2026-08-23-algorithm-strategy.md). |
