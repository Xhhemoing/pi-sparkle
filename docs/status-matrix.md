...
| Capability | Present | Wired | Exercised | Outcome-supported | Notes |
|---|---|---|---|---|---|
| R0 / static `ModelRouter` | yes | live flowchart + `--children` assign | yes | no | Live path. `routeFlowNode` runs `analyzeTask` and persists compile-time `agentRole` so tester/planner do not collapse to implementer (`flowchart-v4`). High-risk analysis now arms the human gate (`WAITING_FOR_USER`); `--assume-defaults` records the wait then auto-selects default approval items (clarify ≠ route approval). F-SIM still assigns the same observed utility to both arms (delta ≡ 0) and now reports selection disagreement as a first-class metric. See [measurement report](reports/2026-08-23-algorithm-measurement.md). |
