...
- **Result:** targeted routing/flowchart/isolation suites **42/42**. `pnpm gate` @ `cbff2d1`: **1201 / 1200 / 0 / 1** (+5 over `865fd30`, all new pins). Fake-executor CLI `--track --assume-defaults` on an isolated temp project:
  - Ordinary `run_af477537-…` COMPLETED, 41 events. Families `plan/research/refactor/review/test` on **both** assign-v5 and flowchart-v5. Scout/tester complexity **LOW on both paths** (A4 closed). planner→premium, others cheap.
  - Deploy `run_cb2efd6c-…` COMPLETED, 43 events. All premium, `highRisk=true`, cheap rejected `complexity` + `high-risk-approval`. Both paths `WAITING_FOR_USER`. 4× `answeredBy: assume-defaults-auto`. A3 line still prints. Sibling risk **not** de-escalated.
  - Local-only exit 1, names `privacy class`. No run.
  - Screenshot `run_c67bba3c-…` COMPLETED, 35 events. planner premium (`preferPrimary`); implementer premium (vision; cheap rejected `capability`); **scout and reviewer cheap** (A2 closed). Both paths agree.
- Stage 2 `--approve-high-risk` still owner-gated. A8 (structured refusal codes) remains queued. No Outcome-supported claim; simulation ≠ F-PROD.
