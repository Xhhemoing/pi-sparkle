...
**D — 运行中**：14 文件（`src/adaptation/` 全树）。基线 `82bef36`。S9-D-4 / S12-D-1 永不再开。默认对照 R31-D（S0c 3.38–3.64 ms）。非再移植：approval-profile constraint-list geometry、SIDEC、PAIRX、CSPELL、ZREP、R31-D 的 R22-D 单元格（`monitor.ts` `windowSize × calibrationDelta`，零生产调用方）、**MODEX**、**BINDX**、I/O text-codec topology / per-run transcode-byte manifest、R32-A 重开条件哨兵审计、R32-B 闭合稳定性审计、R31-I CLI 进程边界门扉闭合审计。若落地：gate + 既有 D harness。报告 `docs/reports/sota-opt/round-32/R32-D.md`。禁止开 PR。

**E — 本波派出**：10 文件（`src/learning/`：auto-loop.ts、learned-routing.ts、from-episode.ts、signals.ts、signatures.ts、bandit-store.ts、task-success.ts、attribution.ts、diagnostics.ts、patterns.ts）。基线 `adb20d7`。默认对照 R31-E（SLICE-CPU cmp25 14.9–15.3）。非再移植：async suspension-point topology / per-run suspension-manifest、PWL、RVL、RTL、CG、JB、NVG、REPRX、PAIRX、ZREP、LINKTOP、**BINDX**、**MODEX**、I/O text-codec topology / per-run transcode-byte manifest、R32-A 重开条件哨兵审计、R32-B 闭合稳定性审计。若落地：gate + 既有 E harness。报告 `docs/reports/sota-opt/round-32/R32-E.md`。禁止开 PR。
...
- 平台硬顶 3 并发云 VM；本波保持 R32-C + R32-D + R32-E。
