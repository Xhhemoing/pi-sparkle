...
状态：第 1 波 A 本波派出。Round 24 I/J 仍在飞。

A 切片 = `src/tracking/` 12 文件 + `src/run/child-tracking.ts` + `src/run/gate-apply.ts`（14）。不要重开 G×E / list-dimension / R22-A 公开导出普查 / R21-A H 输入面 / R20-A openMinors。R24-B M=1 catalog collapse / R24-D approval-authority face / R24-F state serialization-boundary provenance face / R24-H operand-residency census / **JB** / **OCCX** / **CALLB** / **CKMIX** / **RUNLIM** 不要移植。预算 R24-A：67–84 µs/run。基线 `7acb666` 预期空 diff。本波派出。

B 切片 = live 路由 10 文件。Live = R0。不要重开 S12-B-2 / S13-B-1（重开条件未触发）。R24-B M=1 catalog collapse 与 R23-B batch arrangement / R22-B 标识符几何（G×T）不补铸。天花板 R24-B：M=2 8.75–9.07 / M=10 17.78–17.95 ms/eval。基线 `94ed3d9` 预期空 diff。
...
I 切片 = 25 文件。S8-I-1 两臂文件级 blocked。R23-I `.pi/` 可观测树与 R22-I run-record 回读 / R21-I flowchart 平面不补铸。custom−builtin R23-I：children +42.9/+22.0、track +53.5/+25.4、flowchart-flat64 +39.2/+21.6 ms。若落地：重跑 r4i/r5i/r7i（68 / 119 / 80）+ r25i。基线 `8dee7fb` 预期空 diff。R24-I 若先合入，以其收口为准、勿补铸其轴。
