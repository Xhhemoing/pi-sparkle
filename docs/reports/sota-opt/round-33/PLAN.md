...
**目标**：在 R32 十切片诚实空枚举关闭（无 S32-A-* … S32-J-*；R32-A 重开条件哨兵审计 F12a–d 与 R32-B 闭合稳定性审计按方法 / 停驻记录，未铸造新面；R32-J 六个从未受控残余坐标未过三合取，未铸造新面）以及 **FITQ / NAMESHAPE / SCALEX / XPROC / YMIX / SCHEDWIN / KFAN / NVG / SEEDX / TERMCLASS / SHAPEK / ORDX / CKMIX / CALLB / OCCX / JB / RUNLIM / VALCLASS / CG / AGEX / COTARG / PRICEX / RTL / DELINV / PROBX / RVL / SIDEC / PWL / ZREP / PAIRX / CSPELL / REPRX / LINKTOP / BINDX / MODEX** 具名 ban 的前提下，对十切片做**第三十三遍**独立枚举。R32 已 10/10 关闭。本轮状态：**A 已合入；B/C/D 本波派出；E–J 待派**。

**约束**：

- R8–R32 已合入切片全部诚实空枚举。无新 S32-A-* … S32-J-*。R33-A 亦诚实空枚举关闭（无 S33-A-*）。
...
**B — 本波派出**：10 文件（r0 / assign / assign-plan / policy / live-cascade / live-selection / analyze-task / primary-catalog / catalog-model + `src/supervisor/model-router.ts`）。**不要**纳入 `catalog-observed.ts`。基线 `94ed3d9`。默认对照 R32-B（M=2 9.04–9.62 / M=10 17.84–18.43）。非再移植：scale-cube 二阶交互、SCALEX、ORDX、AGEX、YMIX、SHAPEK、REPRX、LINKTOP、R31-B 面格六门停驻、R32-B 闭合稳定性审计（**禁止**再铸「再查一遍闭合稳定性」式新面）、R32-A 重开条件哨兵审计、R33-A 空枚举（A 账本 44/44 卫生，非新面）。S12-B-2 仍未触发。S13-B-1 仍为 loser ID。若落地：gate + 既有 B harness。报告 `docs/reports/sota-opt/round-33/R33-B.md`。禁止开 PR。

**C — 本波派出**：9 文件（r1 / r1-shadow-report / posterior / offline-logit / offline-prob-add / propensity / lin-alg / bandit / shadow）。基线 `183df9b`。默认对照 R32-C（中位 659.0–662.0；组合天花板 20.2–26.6 < 35；sink=7.309）。非再移植：input object-representation / **REPRX**、**BINDX**、KFAN、COTARG、PROBX、DELINV、PRICEX、VALCLASS、R32-A 重开条件哨兵审计、R32-B 闭合稳定性审计、R33-A 空枚举。若落地：gate + r1c–r7c（8028 / 14420 / 14730 / 24888 / 28555 / 25483 / 6193）。报告 `docs/reports/sota-opt/round-33/R33-C.md`。禁止开 PR。

**D — 本波派出**：14 文件（`src/adaptation/`）。基线 `82bef36`。默认对照 R32-D（S0c 3.37–3.60 ms）。非再移植：approval-profile constraint-list geometry、SIDEC、PAIRX、CSPELL、ZREP、R22-D 所有权单元格、R32-A 重开条件哨兵审计（仅卫生应用）、R32-B 闭合稳定性审计、R33-A 空枚举。S9-D-4 / S12-D-1 永不再开。若落地：gate + 既有 D harness。报告 `docs/reports/sota-opt/round-33/R33-D.md`。禁止开 PR。
...
- 平台硬顶 3 并发云 VM；本波保持 R33-B + R33-C + R33-D。
