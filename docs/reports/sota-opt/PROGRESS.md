...
| 40 | 第 2 波进行中 | 1 / 10（A 已合入；B/C 运行中；D 本波派出） | claude-fable-5-thinking-xhigh | [round-40/PLAN.md](./round-40/PLAN.md) |
...
| 8 | R38-J persist | J 已合入 |

### Round 39 波次

| 波 | 区 | 状态 |
| --- | --- | --- |
| 1 | R39-A 跟踪门控、R39-B live 路由、R39-C 离线路由 | A/B/C 已合入 |
| 2 | R39-D 自优化 | D 已合入 |
| 3 | R39-E 学习 | E 已合入 |
| 4 | R39-F 实验 | F 已合入 |
| 5 | R39-G 运行时 | G 已合入 |
| 6 | R39-H 评估 | H 已合入 |
| 7 | R39-I CLI/适配器 | I 已合入 |
| 8 | R39-J persist | J 已合入 |

### Round 40 波次

| 波 | 区 | 状态 |
| --- | --- | --- |
| 1 | R40-A 跟踪门控、R40-B live 路由、R40-C 离线路由 | A 已合入；B/C 运行中 |
| 2 | R40-D 自优化 | D 本波派出 |
...
| R38-A | `cursor/r38-a-tracking-thirty-eighth-pass-83a1` | 已合入专属分支（仅 `R38-A.md`）。无新排除 ID；预算复核 14.0–19.0 µs/gate ⇒ ~70–95 µs/run；三项换名检查拒列未铸 |
| R38-D | `cursor/r38-d-adaptation-thirty-eighth-pass-83a1` | 已合入专属分支（仅 `R38-D.md`）。无新排除 ID；eval 地板复核 S0c 3.47–3.79 ms；零新站点级换名记录 |
| R38-G | `cursor/r38-g-runtime-thirty-eighth-pass-83a1` | 已合入专属分支（仅 `R38-G.md`）。无新排除 ID；计算顶复核 0.291–0.296 vs I/O 87.0–106.1；digest 第二十九次逐位；零新站点级换名记录 |
| R38-J | `cursor/r38-j-persist-thirty-eighth-pass-83a1` | 已合入专属分支（仅 `R38-J.md`）。无新排除 ID；I/O 地板复核 preferences 124.4–255.4 / 400.8–519.0、jsonl 60.6–66.9 / fsync=true 230.0–334.9；J1 2468 / 2951.7×；零新站点级换名记录 |
...
| R39-C | `cursor/r39-c-offline-routing-thirty-ninth-pass-83a1` | 已合入专属分支（仅 `R39-C.md`）。无新排除 ID；中位复核 665.9–693.4（中段宿主）；组合天花板 19.0–30.1 < 35；sink=7.309；零新站点级换名记录 |
| R39-D | `cursor/r39-d-adaptation-thirty-ninth-pass-83a1` | 已合入专属分支（仅 `R39-D.md`）。无新排除 ID；eval 地板复核 S0c 3.46–3.50 ms；零新站点级换名记录 |
| R39-E | `cursor/r39-e-learning-thirty-ninth-pass-83a1` | 已合入专属分支（仅 `R39-E.md`）。无新排除 ID；SLICE-CPU 15.0（默认带沿用 14.7–15.2）；零新站点级换名记录；S8-E-1 仍恰 3 处 |
| R39-F | `cursor/r39-f-experiments-thirty-ninth-pass-83a1` | 已合入专属分支（仅 `R39-F.md`）。无新排除 ID；锚点复核 r5f 128.29/126.03/123.63、r6f 121.06/121.21/131.89、r7f 130.95/126.05/130.38；零新站点级换名记录 |
| R39-G | `cursor/r39-g-runtime-thirty-ninth-pass-83a1` | 已合入专属分支（仅 `R39-G.md`）。无新排除 ID；计算顶复核 0.283–0.302 vs I/O 80.8–99.0；digest 第三十次逐位；零新站点级换名记录 |
| R39-H | `cursor/r39-h-eval-thirty-ninth-pass-83a1` | 已合入专属分支（仅 `R39-H.md`）。无新排除 ID；默认链复核 9201/9008/9247 ns（宿主中窗）；零新站点级换名记录；S5-H-1 保持 |
| R39-I | `cursor/r39-i-cli-thirty-ninth-pass-83a1` | 已合入专属分支（仅 `R39-I.md`）。无新排除 ID；custom−builtin 复核 children +22.8~+24.1 / +41.9~+45.2、track +24.9~+29.2 / +47.8~+54.9、flowchart +12.6~+24.2 / +40.5~+43.9；零新站点级换名记录 |
| R39-J | `cursor/r39-j-persist-thirty-ninth-pass-83a1` | 已合入专属分支（仅 `R39-J.md`）。无新排除 ID；I/O 地板复核 preferences 140.7–226.1 / 404.9–536.5、jsonl 60.4–67.1 / fsync=true 242.2–395.3；J1 2468 / 2929.4×；零新站点级换名记录 |
| R40-A | `cursor/r40-a-tracking-fortieth-pass-83a1` | 已合入专属分支（仅 `R40-A.md`）。无新排除 ID；预算复核 13.8–17.2 µs/gate ⇒ ~69–86 µs/run；三项换名检查拒列未铸 |
| R40-B | `cursor/r40-b-live-routing-fortieth-pass-83a1` | 已派出 |
| R40-C | `cursor/r40-c-offline-routing-fortieth-pass-83a1` | 已派出 |
| R40-D | `cursor/r40-d-adaptation-fortieth-pass-83a1` | 已派出 |
...
| R34-H | `cursor/r34-h-eval-thirty-fourth-pass-83a1` | 已合入专属分支（仅 `R34-H.md`）。无新排除 ID；默认链复核 8989/9021/8926 ns；零新站点级换名记录；S5-H-1 保持 |
...
| R34-I | `cursor/r34-i-cli-thirty-fourth-pass-83a1` | 已合入专属分支（仅 `R34-I.md`）。无新排除 ID；custom−builtin 复核 children +48.1~+54.8 / +25.7~+27.4、track +52.8~+53.7 / +24.5~+29.1、flowchart-flat64 +39.1~+42.9 / +15.6~+34.2；三项换名检查拒列未铸 |
